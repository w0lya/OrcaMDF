...
{
	// The same as Decimal. This is only needed but to keep the exact type name matching.
    public class SqlNumeric : SqlTypeBase
    {
        private readonly byte precision;
		private readonly byte scale;

		public SqlNumeric(byte precision, byte scale, CompressionContext compression)
			: base(compression)
		{
		...
		public override object GetValue(byte[] value)
		{
			if(!CompressionContext.UsesVardecimals)
			{
				...
				var space = getNumberOfRequiredStorageInts();
				int[] ints = new int[4];
